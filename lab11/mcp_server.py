...
def roll_dice(n_dice: int, sides: int, modifier: int = 0) -> str:
    ...
    while dice_count > 0:
        ...
        dice_count -= 1
    final_sum = sum + modifier
    message = f"Rolled {n_dice}d{sides}+{modifier} = {roll_list} + {modifier} = {final_sum}"
    return message
...
def calculate_damage(base_damage: int, armor_class: int, attack_roll: int) -> str:
    ...
    if (attack_roll >= armor_class):
        message = f"Attack hit! {base_damage} dealt!"
    else:
        message = F"Attack missed! No damage dealt!"
    return message
